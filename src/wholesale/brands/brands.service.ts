...
export class BrandsService {
  ...
  async getBrandProducts(
    ...
    // Product IDs filtered by pack variations (colors, sizes)
    let variationProductIds: string[] | null = null;
    if ((colors && colors.length > 0) || (sizes && sizes.length > 0)) {
      let idsByColor: string[] = [];
      let idsBySize: string[] = [];
      ...
      if (colors && colors.length > 0) {
        ...
        if (packVariants && packVariants.length > 0) {
          ...
          const { data: packSizes } = await serviceClient
            ...
            .select('product_id')
            .in('id', packSizeIds);
          if (packSizes) idsByColor = packSizes.map((p: any) => p.product_id);
        }
        if (idsByColor.length === 0) {
          return {
            products: [],
      ...
      if (sizes && sizes.length > 0) {
        ...
        if (packVariants && packVariants.length > 0) {
          ...
          const { data: packSizes } = await serviceClient
            ...
            .select('product_id')
            .in('id', packSizeIds);
          if (packSizes) idsBySize = packSizes.map((p: any) => p.product_id);
        }
        if (idsBySize.length === 0) {
          return {
            products: [],
            pagination: { page, limit, total: 0, totalPages: 0 },
          };
    ...
    // Price range (wholesale_price)
    if (priceRange && priceRange !== 'all') {
      if (priceRange.endsWith('-')) {
        const minVal = parseFloat(priceRange.replace('-', ''));
        if (!isNaN(minVal)) query = query.gte('wholesale_price', minVal);
      } else {
        const parts = priceRange.split('-');
        const minVal = parts[0] ? parseFloat(parts[0]) : NaN;
        const maxVal = parts[1] ? parseFloat(parts[1]) : NaN;
        if (!isNaN(minVal)) query = query.gte('wholesale_price', minVal);
        if (!isNaN(maxVal)) query = query.lte('wholesale_price', maxVal);
      }
    }

    // Min order
    if (minOrder && minOrder !== 'all') {
      const moq = parseInt(minOrder, 10);
      if (!isNaN(moq)) query = query.gte('min_order_quantity', moq);
    }
