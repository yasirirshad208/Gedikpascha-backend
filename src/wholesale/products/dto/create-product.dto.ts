...
export class ProductPackSizeDto {
  ...
  /**
   * When hasFixedQuantities=true: which variation types can the BUYER select?
   * E.g. ["color"] = buyer picks color, size mix (4M+4L+4XL) is fixed.
   * Types NOT in this array are hidden/fixed.
   */
  @IsOptional()
  @IsArray()
  @IsString({ each: true })
  selectableVariationTypes?: string[];
}
