...
export class CartService {
  ...
  async getCart(userId: string) {
    ...
    const transformedItems = (cartItems || []).map((item) => {
      ...
      // Resolve variation-specific image (e.g., show white image when white is selected)
      let displayImage = primaryImage;
      if (item.selected_variations && packSize?.variations) {
        ...
        if (firstKey) {
          ...
          if (colorPart) {
            ...
            // Pack variations use Trendyol-style format with a `color` column (not variation_type)
  ...
  async addToCart(userId: string, addToCartDto: AddToCartDto) {
    ...
    if (!addToCartDto.selectedVariations || Object.keys(addToCartDto.selectedVariations).length === 0) {
      const { data } = await serviceClient
        ...
        .is('selected_variations', null)
        .maybeSingle();
      existingItem = data;
    }

    if (existingItem) {
      // Update existing item (no-variation items only)
  ...
  async syncCart(userId: string, syncCartDto: SyncCartDto) {
    ...
    for (const item of syncCartDto.items) {
      try {
        ...
        if (existingItem) {
          // Merge quantities - keep the higher quantity (no-variation items only)
