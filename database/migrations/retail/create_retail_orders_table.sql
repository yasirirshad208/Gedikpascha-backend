-- Create retail_orders table to store orders
...
-- Create retail_order_items table to store order line items
...
CREATE INDEX IF NOT EXISTS idx_retail_orders_user_id ON retail_orders(user_id);
CREATE INDEX IF NOT EXISTS idx_retail_orders_order_number ON retail_orders(order_number);
...
CREATE INDEX IF NOT EXISTS idx_retail_orders_created_at ON retail_orders(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_retail_order_items_product_id ON retail_order_items(product_id);
...
-- Function to generate retail order number
CREATE OR REPLACE FUNCTION generate_retail_order_number()
RETURNS TEXT AS $$
DECLARE
  new_number TEXT;
  prefix TEXT := 'RO';
...
-- Trigger to auto-generate retail order number
CREATE OR REPLACE FUNCTION set_retail_order_number()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.order_number IS NULL OR NEW.order_number = '' THEN
    NEW.order_number := generate_retail_order_number();
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER set_retail_order_number
  BEFORE INSERT ON retail_orders
  FOR EACH ROW
  EXECUTE FUNCTION set_retail_order_number();

-- Add comments for documentation
COMMENT ON TABLE retail_orders IS 'Retail orders placed by customers';
COMMENT ON TABLE retail_order_items IS 'Line items for each retail order';
COMMENT ON COLUMN retail_orders.order_number IS 'Unique order number in format RO + YYMMDD + sequence';
COMMENT ON COLUMN retail_orders.status IS 'Order status: pending, confirmed, processing, shipped, delivered, cancelled, refunded';
COMMENT ON COLUMN retail_orders.payment_status IS 'Payment status: pending, paid, failed, refunded';

-- Enable Row Level Security (RLS)
ALTER TABLE retail_orders ENABLE ROW LEVEL SECURITY;
ALTER TABLE retail_order_items ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for retail_orders
...
-- Create RLS policies for retail_order_items
...
CREATE POLICY "Users can view own order items"
  ...
  USING (
    EXISTS (
      ...
      WHERE id = retail_order_items.order_id
